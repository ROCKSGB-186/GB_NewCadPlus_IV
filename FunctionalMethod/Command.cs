            ...
            try // 主流程异常捕获块，包裹整个插入逻辑
            {
                ...
                using (var sourceDb = new Autodesk.AutoCAD.DatabaseServices.Database(false, true))
                {
                    ...
                    using (var tr = new DBTrans())
                    {
                        ...
                        var overlapCandidates = InsertGraphicHelper.FindOverlappedCandidates(tr, fileEntity, InsertGraphicHelper._propertySyncMaxCandidates);
                        ...
                        if (overlapCandidates.Count > 0)
                        {
                            ...
                            var activeWhitelist = InsertGraphicHelper.GetActiveWhitelist(fileEntity);

                            // 调用 BuildMergedPropertyMapFromCandidates 从候选图元中合并属性，并应用白名单/黑名单过滤
                            overlapSourcePropertyMap = InsertGraphicHelper.BuildMergedPropertyMapFromCandidates(tr, overlapCandidates, activeWhitelist);
                        ...
                        if (overlapSourcePropertyMap.Count > 0)
                        {
                            ...
                            InsertGraphicHelper.SyncCommonPropertiesToBlockReference(tr, fileEntity, overlapSourcePropertyMap);
                            // 第二步：调用 SyncCommonPropertiesToEntityXRecord 将属性同步到块参照的扩展字典
                            InsertGraphicHelper.SyncCommonPropertiesToEntityXRecord(tr, fileEntity, overlapSourcePropertyMap);
                        ...
                        if (overlapSourcePropertyMap.Count > 0)
                        {
                            ...
                            InsertGraphicHelper.SyncCommonPropertiesToInsertedEntities(tr, insertedEntities, overlapSourcePropertyMap);
            ...
            try
            {
                ...
                using (var tr = new DBTrans())
                {
                    ...
                    var mtext = new MText
                    {
                        ...
                        TextHeight = 2.5 * VariableDictionary.textBoxScale,
