...
namespace GB_NewCadPlus_IV.Helpers // 命名空间与原文件保持一致 // 中文注释
{
    ...
    internal static class PipelineTopologyHelper // 静态类，包含若干静态辅助方法 // 中文注释
    {
        ...
        {
            // 中文注释：事务为空直接返回，避免空引用异常
            if (tr == null) return;
            // 中文注释：新管线ID无效直接返回，避免无效对象访问
            if (newPipeId == ObjectId.Null) return;

            try
            {
                // 中文注释：当前正在处理的管线ID，初始为刚插入的新管线
                ObjectId workingPipeId = newPipeId;
                // 中文注释：是否继续链式并线的开关
                bool continueMerge = true;
                // 中文注释：安全计数器，防止异常数据导致死循环
                int safeLoop = 0;
                // 中文注释：最大循环次数，通常够用且能避免卡死
                const int maxLoop = 30;

                // 中文注释：本地函数-统计字典中非空值数量，用于判断“参数完整度”
                ...
                // 中文注释：外层循环，每并线成功一次就再扫描一轮（支持A-B-C连续并线）
                while (continueMerge && safeLoop < maxLoop)
                {
                    // 中文注释：循环次数+1
                    safeLoop++;
                    // 中文注释：先置为false，只有本轮真的并成功才置回true
                    continueMerge = false;

                    // 中文注释：读取当前工作管线的轴线与元数据
                    ...
                    foreach (ObjectId candidateId in candidates)
                    {
                        ...
                        // 中文注释：定义删除对象ID
                        ObjectId eraseId;
                        // 中文注释：定义需要合并回保留对象的来源属性
                        Dictionary<string, string> inheritMap;

                        // 中文注释：若候选参数更完整，则保留候选并删除当前
                        if (bScore > aScore)
                        {
                            // 中文注释：保留候选对象
                            keepId = candidateId;
        ...
        private static bool TryGetPipeAxisAndMeta(DBTrans tr, ObjectId pipeId, out Point3d start, out Point3d end, out Dictionary<string, string> meta)
        {
            ...
            // 中文注释：ObjectId 为空直接失败
            if (pipeId == ObjectId.Null) return false;

            try
            {
                // 中文注释：按块参照读取，因为你的管线是“PL线制作的属性块”
                ...
                // 中文注释：记录块名到元数据，后续可用于过滤“进口/出口”类型
                ...
                // 中文注释：在块定义里查找“最像管道主体”的 Polyline（优先非闭合，按端点跨度最大）
                Polyline bodyPl = null; // 中文注释：最终选中的主体线
                double bestScore = double.MinValue; // 中文注释：评分越大越优先

                foreach (ObjectId entId in btr) // 中文注释：遍历块定义实体
                {
                    var ent = tr.GetObject(entId, OpenMode.ForRead) as Entity; // 中文注释：读取实体
                    if (ent is not Polyline pl) continue; // 中文注释：只看Polyline

                    if (pl.NumberOfVertices < 2) continue; // 中文注释：顶点不足跳过

                    Point3d p0 = pl.GetPoint3dAt(0); // 中文注释：首点
                    Point3d pN = pl.GetPoint3dAt(pl.NumberOfVertices - 1); // 中文注释：末点

                    double span = p0.DistanceTo(pN); // 中文注释：端点跨度（主体线一般更大）
                    double score = span; // 中文注释：基础分

                    if (!pl.Closed) score += 1000000.0; // 中文注释：强优先非闭合线（箭头通常闭合）

                    if (score > bestScore) // 中文注释：更新最优候选
                    {
                        bestScore = score; // 中文注释：更新评分
                        bodyPl = pl; // 中文注释：更新主体
                    }
                }
